...
void UnTextEdit::openFile()
{
    ...
    this->fileName = QFileDialog::getOpenFileName(this, "Open File", QDir::currentPath(), "Text Files (*.md *.html *.txt *.obj))");
    ...
    if(this->fileName == "")
    {
        ...
        if(isObj) {
            ...
            if(QFile::exists(QFileInfo(fileName).absoluteDir().path() + splittedFile.back().split(".").first() + ".txt"))
            {
                QFile file(QFileInfo(fileName).absoluteDir().path() + splittedFile.back().split(".").first() + ".txt");
